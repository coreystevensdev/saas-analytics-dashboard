...
export function DemoModeBanner({ demoState, onUploadClick }: DemoModeBannerProps) {
  ...
  if (dissolving && dissolveMessage) {
    return (
      <div
        role="status"
        aria-live="polite"
        className="mx-auto max-w-7xl px-4 pt-4 md:px-6 lg:px-8 animate-banner-dissolve motion-reduce:hidden"
      ...
      >
        <div className="flex items-center gap-3 rounded-lg border border-info/30 bg-info/5 px-4 py-3">
          <p className="flex-1 text-sm text-foreground">{dissolveMessage}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      ...
      aria-live="polite"
      className={cn(
        'mx-auto max-w-7xl px-4 pt-4 md:px-6 lg:px-8',
