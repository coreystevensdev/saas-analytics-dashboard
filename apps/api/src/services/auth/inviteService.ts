...
export async function getActiveInvitesForOrg(orgId: number) {
  return orgInvitesQueries.getActiveInvites(orgId);
}
...
export async function generateInvite(orgId: number, createdBy: number, expiresInDays?: number) {
  ...
  const invite = await orgInvitesQueries.createInvite(orgId, tokenHash, createdBy, expiresAt);
...
export async function validateInviteToken(token: string) {
  ...
  const invite = await orgInvitesQueries.findByTokenHash(tokenHash);
...
export async function redeemInvite(inviteId: number, orgId: number, userId: number) {
  const existing = await userOrgsQueries.findMembership(orgId, userId);
  if (existing) {
    // already a member — mark invite used but skip adding membership
    await orgInvitesQueries.markUsed(inviteId, userId);
    logger.info({ orgId, userId }, 'Invite redeemed by existing member — skipped');
    return { alreadyMember: true };
  }

  await userOrgsQueries.addMember(orgId, userId, 'member');
  await orgInvitesQueries.markUsed(inviteId, userId);
